...
def run_extraction(bdf, df_load, df_misc, should_continue, progress_cb):
    ...
    for i, pbarl in enumerate(pbarl_ids, 1):
        ...
        for _, skin_row in df_skin.iterrows():
            ...
            # Only promote deeper_pid to lookup_pid if it's a real shell
            # property (not PBARL, and not some other non-shell property
            # type that happened to share nodes). Otherwise stick with the
            # skin's own PSHELL, which is guaranteed shell-typed.
            if (
                deeper_pid is not None
                and prop_types.get(deeper_pid) in SHELL_PROPERTY_TYPES
            ):
